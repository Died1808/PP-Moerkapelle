...
##############################################K-waarde        
    ...
    def plt(self, filename):
        ...
        df['S'] = np.where(df['depth']> -9, 0.05,  9e-4)
        df['S'] = np.where(df['depth']< -12, 7e-5, df['S'])   
        df['S'] = np.where(df['depth']< -27, 1e-5, df['S'])   

        df['Cc'] = (df['k']/(24*60*60)*df['kzkh']/df['S'])/2.5 #

        ml=Model3D(kaq=df['k'], z=dfn[ 'depth'], Saq= df['S'], kzoverkh=df['kzkh'], tmin=1e-10, 
                   tmax=10, M=10)
        ...
        for rnum in well.index:
            ...
            x    = well['x'][rnum] 
            y    = well['y'][rnum]
            punt = Well(ml, x,y, tsandQ=[(0 ,681.9),(1 , 0*24)], 
                        rw=0.3, layers=well1)


        laag1 = df['depth'].sub(float(-8)).abs().values.argmin() 
        laag2 = df['depth'].sub(float(-15)).abs().values.argmin() 
        laag3 = df['depth'].sub(float(-40)).abs().values.argmin() 
        
        laag= [laag1, laag2, laag3]
        kleuren(len(laag))
        
####################################
        ml.solve()
# ###################################
        onderlaag = abs(dzend)-abs(df.iloc[-1,0])
        KD= round((df['k'].sum()/2.5+onderlaag*df.iloc[-1,2]),0)
        print('KD  =', KD, ' [m2/dag]')
        
        a=np.amin(punt.headinside(1300/1440))
        print('HeadInside = ',round(a,2), ' [m NAP]')
   

        for la in laag:
            ml.xsection(x1=x1, x2=x1, y1=y1, y2=y1+1000, npoints=2500, t=1,
        ...
        plt.yticks(np.arange(0.25,-1.0,-0.05))
        plt.title('Verlaging t.g.v pompproef WAM_1  KD = '+ str(int(round(KD,0)))+' [m2/dag]')
